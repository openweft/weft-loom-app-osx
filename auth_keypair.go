...
// ed25519 private key from the macOS Keychain, sign a fresh assertion
// for the configured gateway, POST it to /api/auth/keypair, and return
// the resulting session token. The cgo Keychain calls live in
// auth_keypair_darwin.go — this file is pure Go so it builds on all
// platforms (the cross-platform desktop variants will reuse it once
// they grow their own keypair store).
//
...
const KeypairKeychainService = "weft-loom-app-keypair"
...
func runKeypair(ctx context.Context, cfg AuthConfig) (Token, error) {
	...
	if fresh {
		...
		// `weft-loom-app-osx 2>>weft-app.log` captures it.

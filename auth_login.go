// auth_login.go : the operator-facing sign-in window. Replaces the
// older 3-button NSWindow picker + child WKWebView pair (auth_darwin.go)
// with a single WKWebView showing the bundled login.html. JS calls Go
// directly via webview_go's Bind, the Go side runs the actual auth
// flow (Keypair / OIDC / OpenPubkey) and destroys the window on
